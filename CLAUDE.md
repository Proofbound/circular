...
**Proofbound brand assets** (logos, favicons, tokens) are served from the shared CDN at `https://brand.proofbound.com/` — never copy them into this repo. [base.njk](src/_includes/base.njk) links the design-system stylesheet `https://brand.proofbound.com/proofbound-design-system/proofbound.css` before the local stylesheet so design-system tokens, fonts, and `.pb-*` classes are available; local [css/style.css](css/style.css) follows in the cascade and preserves Circular's Victorian aesthetic (Playfair Display + Literata, oxblood, ❧ ornaments) — that's intentional, not pending migration. The footer colophon uses `https://brand.proofbound.com/proofbound-design-system/assets/proofbound-logo.svg`. See [ADOPT.md](ADOPT.md) for the full design-system contract; the Victorian look is a deliberate departure from §3 ("Remove hardcoded brand values") in favor of the periodical voice.
